-- ============================================================
-- CREAR_BD_PSFORCE.sql
-- PsForce / ReprediSL V4
-- Ejecutar CONECTADO a la base: repredisl_api
-- PostgreSQL 17 / puerto 5433
-- ============================================================

-- Codificación de cliente para nuevas conexiones a esta BD.
ALTER DATABASE repredisl_api SET client_encoding = 'UTF8';

-- ------------------------------------------------------------
-- 1. Esquema expuesto por PostgREST
-- ------------------------------------------------------------
CREATE SCHEMA IF NOT EXISTS api;

-- ------------------------------------------------------------
-- 2. Roles PostgREST
-- ------------------------------------------------------------
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'web_anon') THEN
        CREATE ROLE web_anon NOLOGIN;
    END IF;
END
$$;

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'authenticator') THEN
        CREATE ROLE authenticator NOINHERIT LOGIN PASSWORD 'Marc';
    ELSE
        ALTER ROLE authenticator WITH NOINHERIT LOGIN PASSWORD 'Marc';
    END IF;
END
$$;

GRANT web_anon TO authenticator;

GRANT USAGE ON SCHEMA api TO web_anon;
GRANT USAGE ON SCHEMA api TO authenticator;

ALTER DEFAULT PRIVILEGES IN SCHEMA api
    GRANT SELECT ON TABLES TO web_anon;

-- ------------------------------------------------------------
-- 3. Pedidos entrantes PWA -> PostgreSQL -> PsGest
-- Basado en Scripts/BaseDatos/CrearTablaPedidos.sql
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.pedidos_nuevos (
    id BIGSERIAL PRIMARY KEY,
    id_empresa INTEGER DEFAULT 1,
    ejercicio INTEGER DEFAULT EXTRACT(YEAR FROM CURRENT_DATE),
    serie VARCHAR(10) DEFAULT '1',
    numero_pedido INTEGER,
    fecha TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    id_cliente BIGINT NOT NULL,
    cliente VARCHAR(255),
    id_forma_pago INTEGER DEFAULT 1,
    id_tarifa INTEGER DEFAULT 1,
    total NUMERIC(12,2) NOT NULL,
    canal VARCHAR(50) DEFAULT 'movil',
    id_vendedor INTEGER DEFAULT 1,
    vendedor VARCHAR(100),
    lineas JSONB NOT NULL DEFAULT '[]'::jsonb,
    estado VARCHAR(20) DEFAULT 'N',
    synced_at TIMESTAMP WITH TIME ZONE DEFAULT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE OR REPLACE VIEW api.pedidos AS
SELECT
    id,
    id_empresa,
    ejercicio,
    serie,
    numero_pedido,
    fecha,
    id_cliente,
    cliente,
    id_forma_pago,
    id_tarifa,
    total,
    canal,
    id_vendedor,
    vendedor,
    lineas,
    estado,
    synced_at,
    created_at
FROM public.pedidos_nuevos;

CREATE OR REPLACE VIEW api.pedidos_nuevos AS
SELECT * FROM api.pedidos;

GRANT ALL ON TABLE public.pedidos_nuevos
    TO postgres, authenticator, web_anon;

GRANT ALL ON SEQUENCE public.pedidos_nuevos_id_seq
    TO postgres, authenticator, web_anon;

GRANT ALL ON api.pedidos
    TO postgres, authenticator, web_anon;

GRANT ALL ON api.pedidos_nuevos
    TO postgres, authenticator, web_anon;

-- ------------------------------------------------------------
-- 4. Permisos para tablas/vistas API que genere la sincronización
-- ------------------------------------------------------------
GRANT SELECT ON ALL TABLES IN SCHEMA api TO web_anon;

-- ------------------------------------------------------------
-- 5. Verificación
-- ------------------------------------------------------------
SELECT current_database() AS base_datos,
       current_user AS usuario_actual;

SHOW server_encoding;
SHOW client_encoding;

SELECT schema_name
FROM information_schema.schemata
WHERE schema_name IN ('public', 'api')
ORDER BY schema_name;

SELECT rolname, rolcanlogin
FROM pg_roles
WHERE rolname IN ('authenticator', 'web_anon')
ORDER BY rolname;

SELECT table_schema, table_name
FROM information_schema.tables
WHERE table_schema IN ('public', 'api')
ORDER BY table_schema, table_name;

SELECT table_schema, table_name
FROM information_schema.views
WHERE table_schema IN ('public', 'api')
ORDER BY table_schema, table_name;
